<template>
  <div class="dashboard-editor-container">
    ...
    <el-row>
      <el-col :span="6" v-for="(asset,index) in this.statisticsData.assetList" :key="index">
        <el-card class="box-card">
          <!--<div slot="header" class="box-card-header">-->
            ...
            <!--</pan-thumb>-->
          <!--</div>-->
          <div slot="header" class="box-card-header" style="text-align: center;">
            <i v-if="asset.assetType==='cpu'" class="iconfont icon-zhuji" style="font-size: 7rem"></i>
            <i v-else-if="asset.assetType==='moniter'" class="iconfont icon-computer" style="font-size: 7rem"></i>
            <i v-else-if="asset.assetType==='laptop'" class="iconfont icon-laptop" style="font-size: 7rem"></i>
            <i v-else="asset.assetType==='ip phone'" class="iconfont icon-telephone2" style="font-size: 7rem"></i>
          </div>
          <span v-if="asset.computerModel===undefined" class="display_name">No this Asset</span>
          <span v-else class="display_name">{{asset.computerModel}}</span>
          <div class="info-item">
            <!--<countTo class="info-item-num" :startVal='0' :endVal='statisticsData.article_count' :duration='3400'></countTo>-->
            <p><span class="info-item-text">assetClass: </span>{{asset.assetType}}</p>
            <p><span class="info-item-text">SerialNumber: </span>{{asset.serialNumber}}</p>
            <p><span class="info-item-text">taggedNumber: </span>{{asset.taggerNumber}}</p>
            <p><span class="info-item-text">beijingCode: </span>{{asset.beijingCode}}</p>
            <p><span class="info-item-text">financeCode: </span>{{asset.financeCode}}</p>
            <p><span class="info-item-text">borrowedDate: </span>{{asset.borrowedDate}}</p>
      ...
      <!--<div class="info-item">-->
      <!--<countTo class="info-item-num" :startVal='0' :endVal='statisticsData.article_count' :duration='3400'></countTo>-->
      <!--<span class="info-item-text">文章aaaaaaaaaaaa</span>-->
      <!--<icon-svg icon-class="a" class="dashboard-editor-icon"></icon-svg>-->
      <!--</div>-->
      <!--<div class="info-item">-->
      <!--<countTo class="info-item-num" :startVal='0' :endVal='statisticsData.article_count' :duration='3400'></countTo>-->
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        statisticsData: {
          ...
          assetList: [{
            assetType :"cpu",
            taggerNumber :'CPE-IT-2016-513',
            beijingCode : 'EQID14082714017',
            financeCode : '101900042985',
            serialNumber : 'PB00DRPN',
            computerModel : 'LenovoP95',
            borrowedDate : '2019-02-11T16:31:11'
          },{
            assetType :"laptop",
            taggerNumber :'CPE-IT-2016-513',
            beijingCode : 'EQID14082714017',
    ...
    methods: {
      getList() {
        getAssetDetails(this.name)
          .then(response => {
            this.statisticsData.assetList = response
          })
      },
    },
    computed: {
